...
def test_domain_filter_allows_platform_policy_with_hot_entities() -> None:
    domain_filter = DomainFilter(threshold=0.45)

    result = domain_filter.check("在最新的平台政策下，如何打造类似 哭哭马 拉布布 的爆款")

    assert result.allow_rag is True
    assert result.score >= 0.45
    assert "平台政策" in result.positive_hits
    assert "爆款" in result.positive_hits
...
def test_planner_uses_query_analyzer_contract_for_web_need() -> None:
    planner = RulePlanner(domain_filter_enabled=False, domain_filter_threshold=0.45)

    output = planner.plan(
        "latest platform policy update",
        trace_context={
            "query_analysis": {
                "need_web_search": True,
                "reason_codes": ["temporal_intent_high", "kb_coverage_low"],
            }
        },
    )

    assert output.need_web_search is True
    assert output.source_route == "hybrid"
    assert output.route_mode == "serial"
    assert output.retrieval_plan["execution_mode"] == "serial"
    assert "temporal_intent_high" in output.reasons
